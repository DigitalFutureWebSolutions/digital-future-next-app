...
const BlogDetails = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await fetch(`https://dfweb-v2.onrender.com/api/v1/api-blogs/${slug}`);
                if (!response.ok) {
    ...
    const meta_keywords = data?.blog?.meta_keyword || '';
                    ...
                    <section className="page-header" style={backgroundStyle}>
                        ...
                        <div className="container">
                            <div className="page-header-content text-center">
                                <h1 className="title text-white">{data?.blog?.title || ''}</h1>
                                <h4 className="sub-title">
                                    <Link className="home" href="/">Home</Link>
                                    <Link className="home" href="/blog">Blog</Link>
                                    <Link className="inner-page custom-heading" href={`/blog/${data?.blog?.slug}`}>{data?.blog?.title || ''}</Link>
                                </h4>
                            </div>
                        </div>
                    </section>
                                ...
                                <div className="col-lg-8 col-md-12">
                                    <div className="sidebar-content-wrap">
                                        <div className="service-details-img">
                                            <img src={data?.blog?.image || ''} alt="blog" />
                                        </div>
                                        <div className="service-details-content">
                                            <h2 className="title custom-heading">{data?.blog?.title || ''}</h2>
